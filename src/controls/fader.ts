...
export class ControlFader extends ControlBase {
	...
	constructor(options: ControlFaderOptions) {
		...
		if (options.dbMap) {
			if (options.dbMap.dbToMidi) {
				this.dbToMidi = options.dbMap.dbToMidi
			}

			if (options.dbMap.midiToDb) {
				this.midiToDb = options.dbMap.midiToDb
			}
		}
	}
	...
	onVariableChange(name: string, value: any): void {
		if (name.includes('-in-percent')) {
			this.sendMidiMode(value, FaderMode.Percent)
		} else if (name.includes('-in-db')) {
			this.sendMidiMode(value, FaderMode.DB)
		}
	}

	onMidiMessage(message: MidiMessage): void {
		super.sendVariableValue(`${super.id}-out-percent`, (message.value / 16383) * 100)
		super.sendVariableValue(`${super.id}-out-db`, this.midiToDb(message.value))
